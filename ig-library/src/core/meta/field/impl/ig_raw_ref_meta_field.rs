...
impl igMetaField for igRawRefMetaField {
    ...
    fn value_from_igz(
        ...
        if _ctx.platform.is_64bit() {
            Some(Arc::new(RwLock::new(read_u64(handle, endian).unwrap())))
        } else {
            Some(Arc::new(RwLock::new(read_u32(handle, endian).unwrap() as u64)))
        }
    }
